...
At the end of the run, before handing back, check the log told the truth. Read `$PI_SESSION_FILE` for the active Pi transcript. For an earlier session, call `pstack_sessions` and read only a returned path; never scan another project's session store. Walk the log against what actually happened:
...
Before handing back, call one `subagent` with `agent: "poteto-agent"`, `role: "judgment and prose"`, and `readonly: true`, and a model different from the one that did the work when configured. Self-review is not a substitute. The child reads the audit trail and the run's transcript, then flags what the user should pay attention to. This is not a redo; it scans for what is suboptimal or risky.
