...
If the checkout doesn't build or start as-is, report it precisely and stop unless fixing startup is also in scope; a skill written against a broken base teaches wrong steps. When an irrelevant missing asset blocks startup (a static dir the API never serves, a sample config), the generated skill may create it in isolated scratch state, clearly marked as verification scaffolding, and remove only that created state in cleanup.
