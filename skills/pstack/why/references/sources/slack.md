...
Slack MCP tools vary. The parent checks which Slack MCP is available and inspects its schema first. If the Pi MCP gateway reports authentication is required, use its documented auth flow. If authentication fails, stop and report the gap.
