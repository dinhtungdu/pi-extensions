...
The parent uses one source-specific playbook per available evidence category. These are concrete examples for common MCPs; adapt them to the actual MCP and inspect its schema first. The parent may pass fetched evidence to isolated subagents with `readonly: true` for reduction, but children do not call MCPs.
