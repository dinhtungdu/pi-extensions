...
You MUST follow the [epistemics framework](epistemics.md). Read it in full before writing the output. The key rules:
...
5. **Verify citations by spot-checking.** You can read the codebase and the parent-provided external evidence to check citations. Do not write files, commit, or modify external state. If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
