...
Do not modify files in the repo. Read code and use only external evidence the parent included with the transcript. Isolated Pi children run without extensions, so identify any ticket, chat, docs, observability, error-tracker, or source-control lookup the parent still needs to perform. The parent agent verifies evidence and applies approved edits.
...
- `read` tool calls against any `SKILL.md` file (workspace `.pi/skills/`, user-level `~/.pi/agent/skills/`, or package skill paths reported by Pi)
