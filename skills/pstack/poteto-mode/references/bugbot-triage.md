...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles Bugbot or review-automation comments. Classification is read-only and grants no forge authority. Without explicit permission to edit, push, reply, or resolve, return the classification and evidence to the parent. The goal is not to ignore Bugbot by default. The goal is to stop treating every comment as a required code change.

## Decision rubric

Classify each Bugbot thread before acting:

- `fix`: The comment identifies a plausible correctness, security, privacy, data loss, auth, billing, migration, idempotency, race, or shipped-behavior issue. Recommend the lowest owning change and verification. The retained parent applies/publishes it and replies only when authorized.
- `dismiss`: The comment matches a documented low-risk noisy pattern, and the current code/context proves the concern does not need a code change. Prepare a short reason. The retained parent replies or resolves only when authorized.
...
Propose new candidate learnings after babysitting when they look reusable. Do not edit this shared skill reference without explicit user approval; route durable lessons through `/skill:reflect`. Promote recurring candidates only after several PRs confirm the pattern.
