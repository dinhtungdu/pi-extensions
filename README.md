...
Manager-owned `manager:task-terminal` events are transported unchanged to exactly one durable session mapping with the same task identity. Relay state records desired receipt and nonce before sending, records successful send before locking, then locks before archiving. Enforced nonce deduplication and idempotent thread mutations recover disconnects or crashes at each boundary without duplicate receipts. Terminal mappings reject reconnect, inbound delivery, wake, and new outbound messages; malformed, unmapped, ambiguous, and cross-task events fail closed.
