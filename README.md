...
During each settled request, native image blocks from finalized Pi tool results are attached to its final text response. The text stays byte-for-byte unchanged; thinking, tool text, progress, user images, Markdown references, and paths are never interpreted as outbound attachments. The relay accepts at most four unique JPEG, PNG, GIF, or WebP images within a 10 MiB total, validates canonical base64, MIME signatures, and size, then persists only mode-0600 relay snapshots and safe digest metadata. Images use relay-generated filenames and attach to the first lossless Unicode-safe text chunk. Missing or corrupt snapshots omit only affected files without changing text. Multipart upload is attempted at most once; uncertain failure is abandoned without text fallback, while later replies continue. Older relays remain text-only without changing response text.
