...
New thread names prefer the Pi session display name, then an explicit title from `TASK.md` at the current checkout/worktree root, then `pi-session-<first-8-session-id>`. A `title:` in leading YAML-style frontmatter takes precedence over the first `#` heading. Discovery reads at most 16 KiB from a regular UTF-8 file and ignores missing, malformed, oversized, binary, or symlinked metadata. Explicit and task-derived names remain capped at 100 characters. Only a thread initially created with the generic fallback is eligible for one conversation-derived rename: after exactly three completed assistant replies, the current Pi model generates a concise title and the bridge attempts one slugged, 100-character-bounded Discord thread rename. The attempt is recorded durably before generation and is never retried after generation or Discord failure, including across restarts. This never changes the Pi session display name, thread identity, or durable mapping. Existing explicit, task-derived, and legacy mappings are ineligible.
