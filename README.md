...
The bridge ignores bot messages and duplicate Discord IDs, and extension-injected Pi input is not echoed back. Interactive Pi user input is mirrored as bold text prefixed with `🖥️`; RPC/non-interactive input is mirrored without that marker. Discord messages received while Pi is active use Pi 0.83's normal `followUp` queue. Assistant text is sent only after `agent_settled`; thinking, tools, progress, attachments, and intermediate responses are not forwarded.
