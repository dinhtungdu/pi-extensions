...
The bridge ignores bot messages and duplicate Discord IDs, and extension-injected Pi input is not echoed back. Each chunk of interactive Pi user input is mirrored unchanged between `──────────── 👨‍💻 ────────────` and `──────────────────────────────` divider lines; RPC/non-interactive input is mirrored without that frame. Discord messages received while Pi is active use Pi 0.83's normal `followUp` queue. Inbound Discord captions and image-only messages preserve that routing and inject up to four JPEG, PNG, GIF, or WebP files through Pi 0.83's native image content. The detached relay alone downloads them from Discord's HTTPS attachment CDN, with an 8 MiB per-image limit, MIME/signature/length checks, three bounded attempts, and no redirects. It persists only bounded metadata and private relay-owned file paths. The authenticated Pi client revalidates those paths and files before base64 encoding; acknowledgement removes durable queue state before safely deleting the local files. Pending files survive client/relay restarts, while old clients remain text-only and cannot acknowledge image messages until upgraded. For Discord-originated prompts, the bot replaces only its own lifecycle reaction: 👀 queued, 🤔 running, ⚙️ using tools, then ✅ settled or ❌ failed/aborted. Reaction failures do not block prompts or replies. Assistant text is sent only after `agent_settled`; thinking, tools, progress, attachments, and intermediate responses are not forwarded.
