...
When Pi runs from a `the-manager` checkout, the extension watches canonical active-task and event changes and publishes one compact task snapshot from `manager status` in that project's parent channel. A latest summary is edited in place. Once displaced, the old bot-owned summary is deleted before its replacement is sent. Desired text, message identity, and send nonce are durable across relay restarts; bounded backoff and nonce deduplication recover delete/send failures without blocking manager mutations or intentionally creating duplicate summaries. Producers submit text only—the relay always derives the project channel from the authenticated session registration.
